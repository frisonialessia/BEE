...
class PendingActionRepository(BaseRepository[PendingAction]):
    ...
    def list_pending(self, limit: int = 50, offset: int = 0) -> list[PendingAction]:
        ...
        stmt = (
            ...
            .order_by(PendingAction.priority.desc(), PendingAction.created_at.asc())  # type: ignore[attr-defined]
            .limit(limit)
            .offset(offset)
        )
        return list(self.session.exec(stmt).all())

    def list_by_opportunity(self, opportunity_id: uuid.UUID) -> list[PendingAction]:
        stmt = (
            select(PendingAction)
            .where(PendingAction.opportunity_id == opportunity_id)
            .order_by(PendingAction.created_at.desc())
        )
        return list(self.session.exec(stmt).all())

    def list_approved(self, limit: int = 50) -> list[PendingAction]:
        """Return approved actions ready to be executed by external tools."""
        stmt = (
            select(PendingAction)
            .where(PendingAction.status == ActionStatus.APPROVED)
            .order_by(PendingAction.priority.desc(), PendingAction.approved_at.asc())  # type: ignore[attr-defined]
            .limit(limit)
        )
        return list(self.session.exec(stmt).all())
    ...
    def count_by_status(self) -> dict[str, int]:
        ...
        results = self.session.exec(select(PendingAction)).all()
        counts: dict[str, int] = {}

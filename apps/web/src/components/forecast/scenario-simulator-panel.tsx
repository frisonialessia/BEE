...
const CHANNELS = ["email", "linkedin", "warm_intro", "twitter"] as const;
...
function VariantCard({ variant }: { variant: ScenarioVariant }) {
  ...
  const t = useTranslations("forecastWinLoss.simulator");
  const color = VARIANT_COLOR[variant.label] ?? "var(--color-chart-4)";
  const variantLabel = t.has(`variants.${variant.label}`) ? t(`variants.${variant.label}`) : variant.label;
  return (
    <div
      className="rounded-[var(--radius-md)] border p-3"
      style={{ borderColor: color, background: `color-mix(in srgb, ${color} 10%, var(--color-background))` }}
    >
      <p className="bee-eyebrow" style={{ color }}>{variantLabel}</p>
      <p className="bee-kpi-sm mt-1" style={{ color }}>
        {formatCurrencyUSDCompact(variant.annual_revenue, locale)}
      </p>
      <p className="bee-caption">
        {t("perMonth", { value: formatCurrencyUSDCompact(variant.monthly_revenue, locale) })}
      </p>
      <p className="bee-caption">
        {t("winRateSuffix", { percent: (variant.win_rate * 100).toFixed(1) })}
      </p>
    </div>
  );
}
        ...
        <div className="bee-bento bee-bento-pad space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <select
              ...
              className="rounded-[var(--radius-md)] border border-border bg-[var(--color-card)] px-3 py-2 text-xs outline-none"
            >
              <option value="">{t("anyDiscStyle")}</option>
              {["D", "I", "S", "C"].map((d) => (
                <option key={d} value={d}>
                  {t("discStyle", { letter: d })}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="space-y-2">
            <label className="block bee-micro">
              {t("monthlySignalsLabel", { value: monthlySignals })}
            </label>
            <input
              type="range"
              min={1}
            ...
            <input
              ...
              className="w-full accent-[var(--color-chart-4)]"
            />
            <label className="block bee-micro">{t("repsLabel", { value: reps })}</label>
            <input
              type="range"
              min={0}
              max={10}
            ...
            <div className="bee-bento bee-bento-pad space-y-2 text-xs">
              <p className="flex items-center gap-1.5 font-medium">
                ...
                <span className="text-muted-foreground">
                  {t("baseWinRate", { percent: (result.base_win_rate * 100).toFixed(1) })}
                </span>
              </p>

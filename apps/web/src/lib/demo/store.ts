 ...
 * a single reached dot; `"16"` adds 460 more filler wins (real Opportunity
 * records, same shape as any other) so her lifetime total clears 500 and
 * the path's higher milestones (200, 500) finally show as reached instead
...
export function resetDemoData(): void {
  const locale = getDemoLocale();
  // Through seedOpportunities (seedOpportunitiesWithReps + the milestone
  // filler wins), not getSampleOpportunities directly: the reseed on
  // version/locale change used to write the raw sample rows, so no
  // opportunity ever carried assigned_to_user_id and the Ranking and quota
  // pace stayed empty in the sandbox for every visitor. This is also the
  // *only* place that actually seeds OPPORTUNITIES_KEY on a fresh visit or
  // a version bump — loadJSON's own seed argument (see `load` above) only
  // ever runs if this key goes missing without a version change, which in
  // practice never happens; both must call the same seedOpportunities() or
  // the two paths quietly seed two different datasets (a real bug this
  // file had until the milestone filler wins were added and reset here
  // still called the old function directly).
  save(structuredClone(seedOpportunities(locale)));
